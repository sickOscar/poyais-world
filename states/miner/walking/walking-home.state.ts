...
export class WalkingHomeState extends WalkingTo implements IState {
    ...
    enter(entity:Miner) {
        ...
        if (hasHouse) {
            movementComponent.arriveOn((<PositionComponent>hasHouse.house.getComponent(PositionComponentName)).position);
        }
    ...
    execute(entity:Miner) {
        ...
        const housePosition = (<PositionComponent>hasHouse.house.getComponent(PositionComponentName)).position;
